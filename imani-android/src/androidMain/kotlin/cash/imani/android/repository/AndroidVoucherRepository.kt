...
class AndroidVoucherRepository(
    ...
    override suspend fun saveVoucher(voucher: StoredVoucher): Result<Unit> = withContext(Dispatchers.IO) {
        runCatching {
    ...
    override suspend fun updateVoucherStatus(id: String, status: VoucherStatus): Result<Unit> = withContext(Dispatchers.IO) {
        runCatching {
            voucherQueries.updateVoucherStatus(status = status.name, voucherId = id)
        }
    }

    override suspend fun deleteVoucher(id: String): Result<Unit> = withContext(Dispatchers.IO) {
        runCatching {
     ...
     */
    suspend fun getPendingSyncVouchers(): Result<List<StoredVoucher>> = withContext(Dispatchers.IO) {
        runCatching {
     ...
     */
    suspend fun updateSyncStatus(id: String, syncStatus: String): Result<Unit> = withContext(Dispatchers.IO) {
        runCatching {
     ...
     */
    suspend fun saveProofs(proofs: List<Proof>, voucherId: String? = null): Result<Unit> = withContext(Dispatchers.IO) {
        runCatching {
     ...
     */
    suspend fun getUnspentProofs(): Result<List<Proof>> = withContext(Dispatchers.IO) {
        runCatching {
     ...
     */
    suspend fun getProofsByVoucher(voucherId: String): Result<List<Proof>> = withContext(Dispatchers.IO) {
        runCatching {
     ...
     */
    suspend fun getProofsByMint(mintUrl: String, unit: String): Result<List<Proof>> = withContext(Dispatchers.IO) {
        runCatching {
     ...
     */
    suspend fun selectProofsForAmount(amount: Long, mintUrl: String, unit: String): Result<List<Proof>> = withContext(Dispatchers.IO) {
        runCatching {
            val allProofs = voucherQueries.selectProofsForAmount(mintUrl = mintUrl, unit = unit)
                .executeAsList()
                .map { it.toProof() }
     ...
     * Mark proof as spent.
     */
    suspend fun markProofAsSpent(secret: String): Result<Unit> = withContext(Dispatchers.IO) {
        runCatching {
            val now = Clock.System.now().toEpochMilliseconds()
     ...
     */
    suspend fun deleteProofs(secrets: List<String>): Result<Unit> = withContext(Dispatchers.IO) {
        runCatching {
