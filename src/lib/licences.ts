...
/**
 * Moved to `@imani/licence` (API ticket 09) so the wallet API can read a
 * licence too, and re-exported because callers here already import it from
 * this path. See `read.ts` there for why it could not simply be imported.
 */
import { licenceOf } from '@imani/licence'
